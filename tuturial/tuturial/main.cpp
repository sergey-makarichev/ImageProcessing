...
void BGR_to_HSV(const Mat& photo, Mat& new_photo)
{
	...
	for (int i = 0; i < photo.rows; i++)
	{
		for (int j = 0; j < photo.cols; j++)
		{
			...
			// ѕреобразуем значени€ в целевой тип
			V *= 255;
			S *= 255;
			H /= 2; // дл€ установки от 0 до 255
...
void HSV_to_BGR(const Mat& photo, Mat& new_photo)
{
	...
	for (int i = 0; i < photo.rows; i++)
	{
		for (int j = 0; j < photo.cols; j++)
		{
			...
			// “еперь от значений H, S, V перейдЄм снова к BGR
			...
			// “еперь, использу€ новые значени€ BGR, сохраним цвет в пикселе
...
int main()
{
	...
	// d)
	int k = 100; // это коэффициент €ркости (не имеет смысла делать меньше -255 или больше 255, так как при <= -255 будет чЄрное изображение, а при >= 255 белое изображение)
	...
	seconds_4 = clock();
	imwrite("hsv_bright_kat.jpg", hsv_bright_kat); // hsv изображение с изменЄнной €ркостью
